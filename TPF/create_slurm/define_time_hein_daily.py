...
time_dir = os.path.join(slurm_dir, 'time')
...
with open(os.path.join(slurm_dir, 'define_all_time_periods' + addendum + '.slurm'), 'w') as all_file:
    ...
    for t in range(len(time_periods)):
        ...
        with open(os.path.join(time_dir, 'define_times' + addendum + '_' + time_periods[t] + '.slurm'), 'w') as file:
            file.write('#!/bin/bash\n')
            ...
            file.write('\n')
            file.write('python '+os.path.join(python_dir, 'define_time_periods_cluster.py')+flags+'\n')
        # Add a line for running the batch script to the overall slurm job.
        all_file.write('sbatch --dependency=singleton ' + os.path.join(
            time_dir, 'define_times' + addendum + '_' + time_periods[t] + '.slurm'))
        all_file.write('\n')