...
with open(os.path.join(project_dir, 'slurm', data_name, 'run_all_TPF.slurm'), 'w') as all_file:
    ...
    for idelta in range(len(deltas)):
        for num_times in [10, 20]:
            ...
            scenarios = {}
            for delta_prior in ["AR", "ART", "RW"]:
                for varfam in ["normal", "MVnormal"]:
                    for rep in range(10):
                        chname = "TPF_" + delta_prior + '_' + varfam + "_K" + str(K) + "_" + str(rep)
            ...
            with open(os.path.join(slurm_dir, 'run_all_TPF.slurm'), 'w') as all_name_file:
                ...
                for chname in scenarios:
                    ...
                    with open(os.path.join(tpf_slurm_dir, chname+'.slurm'), 'w') as file:
                        file.write('#!/bin/bash\n')
                    ...
                    all_name_file.write('sbatch --dependency=singleton '+os.path.join(tpf_slurm_dir, chname+'.slurm'))
                    all_name_file.write('\n')
                    # Add a line for running the batch script to the overall slurm job.
                    all_file.write('sbatch --dependency=singleton ' + os.path.join(tpf_slurm_dir, chname+'.slurm'))
                    all_file.write('\n')
