...
flags.DEFINE_string("addendum",
                    default="",
                    help="String to be put at the end of files.")
flags.DEFINE_integer("senate_session",
                     default=113,
                     help="Senate session (used only when data is 'comparisons').")
...
def main(argv):
    del argv
    project_dir = os.path.dirname(os.path.dirname(os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))))
    print(project_dir)
    ...
    counts_sparse = sparse.load_npz(os.path.join(data_dir, "counts" + FLAGS.addendum + ".npz"))
    counts = np.array(counts_sparse.todense())

    file = open(os.path.join(data_dir, "vocabulary" + FLAGS.addendum + ".txt"))
    ...
    for step in range(FLAGS.max_steps):
        ...
        if step % 50 == 0 or step == FLAGS.max_steps - 1:
            ...
            print("Step: {:>2d}, ELBO: {:.3f} ({:.3f} sec)".format(
                step, elbo, duration))
            np.save(os.path.join(save_dir, "document_shape_K" + str(FLAGS.num_topics) + FLAGS.addendum),
                    variational_document_shape)
            np.save(os.path.join(save_dir, "document_rate_K" + str(FLAGS.num_topics) + FLAGS.addendum),
                    variational_document_rate)
            np.save(os.path.join(save_dir, "topic_shape_K" + str(FLAGS.num_topics) + FLAGS.addendum),
                    variational_topic_shape)
            np.save(os.path.join(save_dir, "topic_rate_K" + str(FLAGS.num_topics) + FLAGS.addendum),
                    variational_topic_rate)
        elif step % 10 == 0:
            duration = time.time() - start_time
