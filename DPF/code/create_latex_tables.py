...
def get_avg_rates(model):
    ...
    ar_ak_cov, ar_ak_var = model.get_ar_cov(model.ar_ak_varfam)
    ar_ak = tfm.exp(model.ar_ak_varfam.location + 0.5 * ar_ak_var)
    ...
    for t in range(model.num_times):
        ar_ak_t = tf.gather(ar_ak, t, axis=-1)
        ar_kv_t = tf.gather(ar_kv, t, axis=-1)
        avg = tfm.reduce_mean(ar_ak_t, axis=0) * tfm.reduce_mean(ar_kv_t, axis=1)  # [K]
