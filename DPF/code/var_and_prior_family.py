...
def cov_from_AR1_prec_matrix(delta, tau, T):
    ...
    diagdelta2 = tf.repeat(delta[:, :, :, tf.newaxis] * delta[:, :, :, tf.newaxis], T - 1, axis=-1)
    replacement_slice = tf.fill(tf.shape(diagdelta2)[:-1], 0.0)
...
class PriorFamily(tf.keras.layers.Layer):
    ...
    def get_log_prior(self, samples):
        ...
        if samples is not None:
            ...
            log_prior = tf.fill([samples.shape[0]], 0.0)
        return log_prior
