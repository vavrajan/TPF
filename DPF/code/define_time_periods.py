...
def define_time_periods_hein_daily(data_dir, addendum='', time_periods='sessions', author='author',
                                   min_word_count=1, min_word_count_per_time=False):
    ...
    if author == 'author':
        ...
        # Author==state --> instead of author id use his/her state label
        # Create index of author, time
        speech_info['state'] = author_info['state'][speech_info['index']].reset_index(drop=True)
        idname = 'state'
        ...
        speech_info['state'] = author_info['state'][speech_info['index']].reset_index(drop=True)
        idname = 'state' # 'state2'
        # todo Each state should have two senators! --> two labels per state
    else:
        raise ValueError('Unrecognized choice for the label of the author.')

    authors = sorted(speech_info[idname].unique())
    authors_index = pd.DataFrame(range(len(authors)), index=authors, columns=['ind'])
    speech_info['author'] = authors_index.loc[speech_info[idname]].reset_index(drop=True)['ind'].to_numpy()
    speech_info['time'] = pd.cut(speech_info['date'], bins=breaks, right=False, labels=False)
    num_authors = len(authors)

    # Create author_time_info data_frame and author_time index
    # + Sum counts over authors and times
    weights, dsbreaks, dbreaks = get_time_weights(session_breaks, breaks)
    author_time_info = pd.DataFrame()
    counts_avt = np.zeros((num_authors, num_words, num_times))
    for ia in range(num_authors):
        a = authors[ia]
        adata = author_info[author_info[idname] == a]
        ...
        for t in range(num_times):
            row = adata[adata['session'] == closest_session_for_time[t] + 97].mode().loc[[0]]
            row['author'] = ia
            row['time'] = t
            sub_counts = counts[((speech_info['author'] == ia) & (speech_info['time'] == t)), :]
            row['presence'] = (sub_counts.shape[0] > 0)
            counts_avt[ia, :, t] = sub_counts.sum(axis=0)
    ...
    # keep only data about present authors:
    sub_author_time_info = author_time_info[author_time_info['presence']].reset_index(drop=True)
    num_author_times = sub_author_time_info.shape[0]
    counts_av = np.zeros((num_author_times, num_words))
    for at in range(num_author_times):
        ia = sub_author_time_info["author"].loc[at]
        t = sub_author_time_info["time"].loc[at]
        counts_av[at, :] = counts_avt[ia, :, t]
    ...
    # sparse.save_npz(os.path.join(data_dir, 'counts_avt' + addendum + '_' + time_periods + '.npz'),
    #                 sparse.csr_array(counts_avt).astype(np.float32))
    np.save(os.path.join(data_dir, 'counts_' + author + 'vt' + addendum + '_' + time_periods), counts_avt)
    np.save(os.path.join(data_dir, 'counts_' + author + 'v' + addendum + '_' + time_periods), counts_av)
    np.savetxt(os.path.join(data_dir, 'vocabulary'+addendum+'_'+time_periods+'.txt'), vocabulary, fmt="%s")
    speech_info.to_csv(os.path.join(data_dir, 'speech_info_'+author+addendum+'_'+time_periods+'.csv'), index=False)
    author_time_info.to_csv(os.path.join(data_dir, 'author_time_info_'+author+addendum+'_'+time_periods+'.csv'), index=False)
...
def define_time_periods(data_name, data_dir, addendum, time_periods, min_word_count, min_word_count_per_time):
    ...
    if data_name == "hein-daily":
        define_time_periods_hein_daily(data_dir, addendum, time_periods, 'author', min_word_count, min_word_count_per_time)
        define_time_periods_hein_daily(data_dir, addendum, time_periods, 'state', min_word_count, min_word_count_per_time)
        define_time_periods_hein_daily(data_dir, addendum, time_periods, 'state2', min_word_count, min_word_count_per_time)
    else:
